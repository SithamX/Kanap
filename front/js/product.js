...
function addSelectionCart(){
    ...
    if (localStorageP) {
      ...
      // on crée une constante qui permettra de vérifier si la sélection effectuée concernerait un produit ayant déjà son id en plus de sa couleur de présents dans le localStorage ;
      const changeQuantity = localStorageP.find(element => element.id == cartSelection.id && element.color == cartSelection.color);

      // si la sélection effectuée concernerait un produit ayant déjà son id en plus de sa couleur de présents dans le localStorage, alors on ne modifie que la quantité puis on l'ajoute au localStorage ;
      if (changeQuantity) {
